...
let max_rgb = 255
let mandelbrotTest (c:Complex) z = Complex.Pow(z, 2.0) + c // This is the actual Mandelbrot set "algorithm"
...
let colorMod iteration =
    ...
    Color.FromArgb(interprolatedColorValue, interprolatedColorValue, interprolatedColorValue) // Maybe we can enhance this to return pretty colors in the future, but grey-scale looks good for now.

[<EntryPoint>]
let main _ =
     let startTime = System.DateTime.Now
     let p0 = new Complex(0.0, 0.0)
     let plotScalingFactor = 1000.0 // This essentially controls how large/detailed the final bitmap will be. I can only get this up to about 5000 on a 64bit windows system with 64gb of ram. This is probably limited by System.Drawing.Bitmap.
     let stepSize = 1.0 / plotScalingFactor
     let plotsize = 2.0 // This should always be 2.0 because when you plot the Mandelbrot set on a coordinate plane, its range on the x-axis is -1 to 2.0.
